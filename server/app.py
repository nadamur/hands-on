...
def prediction_text():
    print("Prediction text function is running!")
    while True:
        if sentence:
            sentence_str = 'Sentence: '.join(sentence) + '\r\n\r\n'
            print('Emitting sentence:', sentence_str)
            socketio.emit('sentence', {'sentence': sentence_str})
            sentence.clear()
        else:
            time.sleep(0.1)  # Add a small delay if there's no new information to avoid busy-waiting
...
if __name__ == '__main__':
    ...
    socketio.start_background_task(target=prediction_text)
